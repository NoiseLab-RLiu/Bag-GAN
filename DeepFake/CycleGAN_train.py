...
#G : X->Y
G = net.Generator(im_in_channels, im_out_channels, im_size)

#F : Y->X
F = net.Generator(im_out_channels, im_out_channels, im_size)

#G Generator discriminator
...
if not os.path.isdir("./loss/" + dataset + "/"):
    os.mkdir("./loss/" + dataset + "/")
    
if os.path.isfile("./loss/" + dataset + "/loss.log") and not load_partial_net:    
    os.remove("./loss/" + dataset + "/loss.log")
    
logfile = open("./loss/" + dataset + "/loss.log","a")
